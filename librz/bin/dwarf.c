...
static void store_line_sample(RzBinSourceLineInfoBuilder *bob, const RzBinDwarfLineHeader *hdr, RzBinDwarfSMRegisters *regs,
	...
	if (regs->file_prev != regs->file) {
		// push only a new file sample if it differs from the previous
		regs->file_prev = regs->file;
		char *full_file = NULL;
		if (regs->file) {
			ut64 fnidx = regs->file - 1;
			full_file = rz_bin_dwarf_line_header_get_full_file_path(info, hdr, fnidx);
		}
		rz_bin_source_line_info_builder_push_file_sample(bob, regs->address, full_file);
		free(full_file);
	} else if (!regs->line) { // without a line, we close the file record too
		rz_bin_source_line_info_builder_push_file_sample(bob, regs->address, NULL);
	}
	if (regs->file) {
		rz_bin_source_line_info_builder_push_line_sample(bob, regs->address, (ut32)regs->line, (ut32)regs->column);
	} else {
		// if there is no file anymore, this should be a closing entry
		rz_bin_source_line_info_builder_push_line_sample(bob, regs->address, 0, 0);
	}
}
...
RZ_API bool rz_bin_dwarf_line_op_run(const RzBinDwarfLineHeader *hdr, RzBinDwarfSMRegisters *regs, RzBinDwarfLineOp *op,
	RZ_NULLABLE RzBinSourceLineInfoBuilder *bob, RZ_NULLABLE RzBinDwarfDebugInfo *info) {
	...
	case RZ_BIN_DWARF_LINE_OP_TYPE_STD:
		...
		case DW_LNS_copy:
			if (bob) {
				store_line_sample(bob, hdr, regs, info);
			}
			regs->basic_block = DWARF_FALSE;
			break;
		case DW_LNS_advance_pc:
...
static size_t parse_opcodes(const ut8 *obuf,
	...
	while (buf < buf_end) {
		...
		if (bob) {
			rz_bin_dwarf_line_op_run(hdr, regs, &op, bob, info);
		}
		if (ops_out) {
...
static RzBinDwarfLineInfo *parse_line_raw(RzBinFile *binfile, const ut8 *obuf,
	...
	while (buf <= buf_end) {
		...
		if (buf_size > unit->header.unit_length) {
			buf_size = unit->header.unit_length + (unit->header.is_64bit * 8 + 4); // we dif against bytes_read, but
		...
		do {
			...
			tmp_read = parse_opcodes(buf, buf_size - bytes_read, &unit->header,
				...
				info, big_endian, target_addr_size);
			bytes_read += tmp_read;
