...
typedef struct {
	...
	/**
	 * Not an actual register but keeps the last file index
	 * stored as line info to not emit more file entries than necessary.
	 */
	ut64 file_prev;
...
RZ_API bool rz_bin_dwarf_line_op_run(const RzBinDwarfLineHeader *hdr, RzBinDwarfSMRegisters *regs, RzBinDwarfLineOp *op,
	RZ_NULLABLE struct rz_bin_source_line_info_builder_t *bob, RZ_NULLABLE RzBinDwarfDebugInfo *info);
